...
> **Not-yet-end-to-end-buildable posture — residual blockers.** The DP-2 035
> runtime for `consoleApplyPayment` + the two receivable reads is **present**
> (merged on DP-2 `origin/main` @ `cb44d4f`, verified 2026-06-16 — controller +
> services + `SettlementModule` + migration 0027), so it is **not** a blocker. The
> build phases (T-codegen onward) are therefore **GATED on 019's own `G-client`
> (generate the clients) + `G-boundary` (boundary test) AND on `G-runtime-032`/G7**:
> DP-2 must confirm the 032 end-to-end posting-retry (G7) path is wired so
> `consoleRepairSaleSync` runs end-to-end — the `consoleRepairSaleSync` controller
> is already present on DP-2 `main`, but the end-to-end G7 path is unverified ⇒
> treated as a blocker — before the 032 build can complete. 019 is the only 035
> child carrying that 032 runtime gate. The 035 ops are also **not yet activated**
> (contract `1.0.0-draft`; migration 0027 G3 is an open human gate). Until those
> clear, only the design
...
- [ ] T003 In `research.md`, record the **residual-gate** posture (load-bearing):
  the three consumed 035 ops are **contract-present** in `settlement/settlement.yaml`
  (`1.0.0-draft`) **AND runtime-backed** — the DP-2 035 runtime is merged on
  `origin/main` @ `cb44d4f` (controller + services + `SettlementModule` + migration
  0027, verified 2026-06-16), so the 035 runtime is **not** a blocker (root cause of
  the earlier "runtime-absent" reading: a stale `settlement.yaml` header comment —
  spec CL-9); the 035 residual is 019's own `G-client` / `G-boundary`. AND the 032
  `consoleRepairSaleSync` **controller is present on DP-2 `main`** (verified) but its
  **end-to-end posting-retry (G7) wiring is unverified ⇒ treated as a blocker** (gate
  rule: uncertain ⇒ blocker). Define the residual blocking gates (`G-client`,
  `G-boundary`, `G-runtime-032`/G7) and state 019 cannot build end-to-end until they
  clear. Note the 035 ops are also **not yet activated** (contract `1.0.0-draft`;
  migration 0027 G3 is an open human gate) — an activation caveat, not a missing
  runtime.
...
## Phase 2 — Codegen *(this phase IS 019's `G-client`; T008/035 can run against the present 035 runtime, T009/032 is GATED on `G-runtime-032`/G7)*

- [ ] T008 *(G-client — 035 runtime present)* Add the DP-2 `settlement/settlement.yaml`
  source to the Console codegen config at a **pinned codegen SHA** (the SHA at which
  DP-2's merged 035 runtime sits — present on `origin/main` @ `cb44d4f`); regenerate
  the generated client.
- [ ] T009 *(gated on G-runtime-032/G7)* Add the DP-2 `sale-sync-ops/sale-sync-ops.yaml` source to the
...
## Phase 3 — Data layer *(GATED on `G-client`/`G-boundary` AND `G-runtime-032`/G7)*
...
## Phase 4 — Surfaces *(GATED on `G-client`/`G-boundary` AND `G-runtime-032`/G7)*
...
## Phase 5 — Tests *(GATED on `G-client`/`G-boundary` AND `G-runtime-032`/G7)*
...
## Phase 6 — Validation *(GATED on `G-client`/`G-boundary` AND `G-runtime-032`/G7)*
...
- Any end-to-end build before 019's own `G-client` / `G-boundary` are done AND the
  DP-2 032 end-to-end posting-retry (G7) wiring is verified (§3.4 of spec). The DP-2
  035 runtime already exists @ `cb44d4f` and is not a blocker.
