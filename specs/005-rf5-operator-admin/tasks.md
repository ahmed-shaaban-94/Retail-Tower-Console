...
- [ ] T002 🔗 SHARED-FILE **(OQ-5)** Add `packages/contracts/openapi/tenants.openapi.yaml` + `memberships.openapi.yaml` to `OPENAPI_SOURCES` in `openapi-ts.config.ts`, then re-run `pnpm generate:client` at pin `62d0906` so `src/generated/schema.d.ts` includes `/api/v1/tenants/{tenant_id}/members`, `/api/v1/memberships/invite`, `/api/v1/memberships/{membership_id}`, and `/api/v1/invitations/accept`. Regeneration at the EXISTING pin (NOT a re-pin); editing `openapi-ts.config.ts` is a slice-002 scaffold-config touch — acknowledge at the FR-008 gate, and sequence vs sibling slices (each adds its own contract to the same config). Do not hand-edit the generated output (slice 002 D-7). Verify the four paths appear.
