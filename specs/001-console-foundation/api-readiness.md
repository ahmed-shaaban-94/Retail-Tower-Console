...
| Generated-client toolchain + storage location | `draft` (toolchain + storage decided and committed; generated **output is stubbed**, not yet produced from live contracts — `draft` per the legend: "exists but is incomplete / in active change") | **Toolchain: `openapi-typescript` + `openapi-fetch`** (D-2). **Storage: vendored at `src/generated/`, committed** (D-7). Decided in slice `002-tooling-and-scaffold` `/speckit-clarify` (`e766a76`) and implemented in slice 002 (`openapi-ts.config.ts` pins Data-Pulse-2 `main` @ `62d0906`; `generate:client` script present). **⚠️ The committed `src/generated/schema.d.ts` is a hand-checked STUB covering RF-1's seven operations — NOT yet generated from the live contracts** (Data-Pulse-2 was not reachable from the implementing workspace). Run `pnpm generate:client` against `62d0906` to replace the stub with real output. | 2026-05-30 (toolchain + storage chosen and committed; output stubbed pending live regeneration) | Ahmed Shaaban | OQ-6 toolchain choice is now **resolved** (was gated by FR-006 → decided in slice 002 per the clarify). **OQ-002-1 confirmed:** no upstream client package ⇒ 002 generates locally and vendors output (the foreclosed "consume a package" branch). **Pin:** 002 re-pinned C-4 `b5142fe` → `62d0906`; the generator config targets that SHA. **Stub caveat (do not over-read this row):** the *toolchain and storage location* are real and committed; the *generated output* is a stub until `pnpm generate:client` runs against reachable contracts. The RF-1 implementation slice (`003-rf1-auth-shell`) MUST regenerate before relying on the client. |
